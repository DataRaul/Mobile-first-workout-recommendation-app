...
During a workout the existing set-entry card is before exercise media, technique and notes. Exercise name, set context, prescription, the compact **Last** performance line, weight, reps, RIR and the complete-set action are therefore the first training controls. The current incomplete set is highlighted, decimal loads remain supported, and a completed set prefills the next blank set's load without changing completed history.

Crowded-gym execution order is flexible without changing the accepted routine. A collapsed **Today's exercises** queue shows not-started, in-progress and completed status, and any planned exercise can be selected directly. **Skip for later** first persists the visible set inputs, then opens a full-day exercise picker so the user explicitly chooses what to do next; the skipped exercise remains unfinished or in progress and selectable later. **Finish workout** is explicit and separate from exercise position, so reaching the final planned exercise cannot accidentally complete an unfinished session; the existing partial-workout confirmation remains the only intentional partial finish path.
