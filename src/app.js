...
function profileComplexityText() {
  const cap = maxComplexity(state.profile?.level);
  return `${LEVELS[state.profile?.level] || "Starter"} profile · exercises up to ${cap}/4 (${COMPLEXITY_LABELS[cap]})`;
}
...
function renderOnboarding(edit = false) {
  ...
  $("#onboardingView").innerHTML = `
    ...
    <form id="profileForm" class="card grid">
      ...
      <fieldset>
        ...
        <p class="notice">Choose a starting split, then customise any day. A workout type keeps the broad structure; priority muscles are selected earlier and repeated when session length allows. Choose up to three priority muscle groups per day.</p>
  ...
  function renderWorkoutDayEditor() {
      ...
      .map(
        (day, index) => `
          <article class="card workout-day-card">
            ...
            <fieldset>
              <legend>Priority muscles</legend>
    ...
    $$('[data-day-field="type"]').forEach((select) => {
      select.addEventListener("change", (event) => {
        selectedWorkoutDays[Number(event.target.dataset.dayIndex)].type = event.target.value;
        markStructureCustom();
      });
    });
  ...
  $("#profileForm").addEventListener("submit", (event) => {
    ...
    const invalidCustomDay = workoutDays.find(
      (day) => day.type === "custom" && !day.emphasis.length,
    );
    if (invalidCustomDay) {
      alert(`Custom workout “${invalidCustomDay.name}” needs at least one priority muscle group.`);
      return;
    }
    ...
    <div class="programme-workout">
      <div class="summary-row">
        <div>
          ...
          <p>${workout.exercises.length} exercises · approximately ${state.profile.sessionMinutes} minutes${workout.emphasis?.length ? ` · focus: ${workout.emphasis.map(labelize).join(", ")}` : ""}</p>
        </div>
      </div>
      ${workout.exercises
        .map((item, index) => {
          ...
          return `
            <div class="exercise-line">
              ...
              <div>
                ...
                <small class="complexity-meta">${escapeHtml(complexityText(exercise))}</small>
              </div>
...
function renderPlanner() {
  ...
  $("#plannerView").innerHTML = `
    ...
    <div class="card">
      <h2>Why this fits</h2>
      <p>The routine uses your goal, experience, available equipment and active safety filters. Exercise complexity never exceeds the level selected in your profile.</p>
...
function replacementContext({ scope, workoutId, itemIndex }) {
  if (scope === "draft") {
    ...
    return {
      item: workout.exercises[itemIndex],
      existingIds: workout.exercises.map((entry) => entry.exerciseId),
      apply(replacementId) {
        workout.exercises[itemIndex].exerciseId = replacementId;
        markDraftUpdated();
        persist();
        renderPlanner();
      },
  ...
  if (scope === "session") {
    ...
    return {
      ...
      apply(replacementId, { permanent = false, markUnavailable = false } = {}) {
        ...
        if (permanent) {
          ...
          if (template) template.exerciseId = replacementId;
        }
